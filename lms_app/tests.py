...
class LessonCRUDTests(APITestCase):
    def setUp(self):
        ...
        self.lesson = Lesson.objects.create(title="Test Lesson", description="Test Lesson Description", course=self.course, owner=self.user)
    ...
    def test_lesson_retrieve(self):
        ...
        response = self.client.get(url)
        print(
            f"URL: {url}, Response status code: {response.status_code}, Response data: {response.data}")  # Отладочный вывод
        self.assertEqual(response.status_code, status.HTTP_200_OK)
