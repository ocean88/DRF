...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        course = Course.objects.create(
            title="Sample Course", description="Description for Sample Course"
        )

        # Создаем урок для курса
        lesson = Lesson.objects.create(
            title="Sample Lesson",
            description="Description for Sample Lesson",
            course=course,
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Sample course, lesson, and payment records created successfully"
            )
        )
